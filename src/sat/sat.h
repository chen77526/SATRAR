...
class SatSolver
{
   public : 
      ...
      // void assumePopBack() { _assump.pop(); }
      void assumeProperty(Var prop, bool val) {
         _assump.push(val? Lit(prop): ~Lit(prop));
      }
      bool assumpSolve() { return _solver->solve(_assump); }
      bool MASolve(map<unsigned, unsigned>& MA_wt, vector<unsigned>& dominators) { return _solver->solveMA(_assump, MA_wt, dominators); }
      bool MASolveTwo(map<unsigned, unsigned>& MA_wt, vector<unsigned>& dominators, vector<unsigned>& inputs) { return _solver->solveMATwo(_assump, MA_wt, dominators, inputs); }
      bool ConflictSolve(vector<unsigned>& dominators) { return _solver->solveConflict(_assump, dominators); }
      void resetAssign() { _solver->resetAssign(); }
      void printAssign() { _solver->printAssign(); }
      void printAssumption() { printf("assumptions: \n"); for(size_t i=0; i<_assump.size(); ++i) { printf("%d\n", _assump[i]);}}
      // void undoAssign(int last)  { _solver->undoAssign(last);}
